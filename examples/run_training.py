...
def load_foodwaste_dataset(embedding_model: FeatureExtractor):
    ...
    logger.info(f"Loaded FoodWaste dataset: {len(train_dataset)} train, {len(val_dataset)} val samples")
...
def load_food_dataset(embedding_model: Optional[FeatureExtractor] = None):
    ...
    logger.info(f"Loaded Food-101 dataset: {len(train_dataset)} train, {len(val_dataset)} val samples")
    return train_dataset, val_dataset
...
def main():
    ...
    try:
        ...
        logger.info(f"Training configuration: {config.num_classes} classes, {config.epochs} epochs")
