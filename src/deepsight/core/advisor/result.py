...
class AdvisorResult(BaseModel):
    ...
    artifacts_failed: List[str] = Field(
        default_factory=list,
        description="List of artifact keys that failed to load"
    )
    
    # Query information
    prompt_generated: Optional[str] = Field(
        default=None,
        description="Generated prompt from artifacts"
    )
    prompt_length: Optional[int] = Field(
        default=None,
        description="Length of the generated prompt in characters"
    )
    
    # Intelligence response
    response: Optional[IntelligenceResponse] = Field(
        default=None,
        description="Response from intelligence provider"
    )
    response_content: Optional[str] = Field(
        default=None,
        description="Text content of the intelligence response"
    )
    response_length: Optional[int] = Field(
        default=None,
        description="Length of the response content in characters"
    )
    ...
    artifact_loading_time: float = Field(
        default=0.0,
        description="Time spent loading artifacts in seconds"
    )
    query_generation_time: float = Field(
        default=0.0,
        description="Time spent generating query in seconds"
    )
    intelligence_execution_time: float = Field(
        default=0.0,
        description="Time spent executing intelligence query in seconds"
    )
    
    # Output information
    output_paths: Dict[str, Path] = Field(
        default_factory=dict,
        description="Paths to saved output files"
    )
    ...
    def add_artifact_loaded(self, artifact_key: Union[str, ArtifactPaths]) -> None:
        ...
        key_str = artifact_key.value if hasattr(artifact_key, 'value') else str(artifact_key)
        if key_str not in self.artifacts_loaded:
            self.artifacts_loaded.append(key_str)
    
    def add_artifact_failed(self, artifact_key: Union[str, ArtifactPaths], error: str) -> None:
    ...
    def set_response(self, response: IntelligenceResponse) -> None:
        ...
        self.response_length = len(self.response_content) if self.response_content else 0
    ...
    def to_dict(self, include_content: bool = True) -> Dict[str, Any]:
        ...
        result_dict['output_paths'] = {
            k: str(v) for k, v in self.output_paths.items()
        }
    ...
    def to_json(self, file_path: Optional[Union[str, Path]] = None, include_content: bool = True) -> str:
    ...
    def to_yaml(self, file_path: Optional[Union[str, Path]] = None, include_content: bool = True) -> str:
