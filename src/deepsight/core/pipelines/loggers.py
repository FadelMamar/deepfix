...
from ..artifacts import ArtifactsManager,ArtifactPath,DeepchecksArtifacts,DatasetArtifacts,TrainingArtifacts
...
class LogArtifact(Step):
    def __init__(self,artifact_key:ArtifactPath,sqlite_path:str,mlflow_manager:MLflowManager):
...
class LogTrainingArtifact(LogArtifact):
    def __init__(self,sqlite_path:str,mlflow_manager:MLflowManager):
        super().__init__(artifact_key=ArtifactPath.TRAINING,sqlite_path=sqlite_path,mlflow_manager=mlflow_manager)

    def run(self,context:dict,metric_names:List[str]=None,)->dict:
...
class LogChecksArtifacts(LogArtifact):
    def __init__(self,sqlite_path:str,mlflow_manager:MLflowManager):
        super().__init__(artifact_key=ArtifactPath.DEEPCHECKS,sqlite_path=sqlite_path,mlflow_manager=mlflow_manager
                    )
...
class LogModelCheckpoint(LogArtifact):
    def __init__(self,mlflow_manager:MLflowManager,sqlite_path:str):
        super().__init__(artifact_key=ArtifactPath.MODEL_CHECKPOINT,sqlite_path=sqlite_path,mlflow_manager=mlflow_manager
                    )
    def run(self,context:dict,checkpoint_artifact_path:Optional[str]=None)->dict:
...
class LogDatasetMetadata(LogArtifact):
    def __init__(self,sqlite_path:str,mlflow_manager:MLflowManager,dataset_name:str):
        super().__init__(artifact_key=ArtifactPath.DATASET,sqlite_path=sqlite_path,mlflow_manager=mlflow_manager
                    )
    ...
    def run(self,
        ...
        assert isinstance(dataset_name, str), f"dataset_name must be a string, got {type(dataset_name)}"
        assert isinstance(train_data, Dataset), f"train_data must be a PyTorch Dataset, got {type(train_data)}"
        assert isinstance(test_data, Dataset), f"test_data must be a PyTorch Dataset, got {type(test_data)}"
