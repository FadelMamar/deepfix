...
class ClassificationVisionDataLoader:
    ...
    @classmethod
    def load_from_dataset(cls,dataset: Dataset,batch_size: int = 8,shuffle: bool = True,model: Optional[torch.nn.Module] = None) -> VisionData:
        assert isinstance(dataset,Dataset), "dataset must be an instance of torch.utils.data.Dataset. Received: {}".format(type(dataset))
        collate_fn = partial(classification_collate_with_model,model=model) if model else classification_collate
