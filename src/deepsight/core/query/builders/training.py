...
class CurvePlotter:
    ...
    def _set_df_metrics(self,metrics_values:pd.DataFrame):
        ...
        df_metrics["key"] = df_metrics["key"].apply(lambda x: 'loss' if 'loss' in x else x)
        # Create the figure and axes
        self.fig,self.ax = plt.subplots(figsize=(15,10),nrows=math.ceil(len(df_metrics)/2),ncols=2,sharex=True)
    ...
    def plot(self,df_metrics:pd.DataFrame,save_path=None):
        ...
        for i, (name,df) in enumerate(df_metrics.groupby('key')):
            if name == 'loss':
                ...
                self._plot_val_curves(df,y_col='value',x_col='step',ax=self.ax[i],title='F1 Score')
            elif name == 'val_precision':
                self._plot_val_curves(df,y_col='value',x_col='step',ax=self.ax[i],title='Precision')
            elif name == 'val_recall':
                self._plot_val_curves(df,y_col='value',x_col='step',ax=self.ax[i],title='Recall')
