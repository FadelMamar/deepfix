...
class Result(BaseModel):
    ...
    def to_dict(self, exclude:list[str]=['value','params','link_in_summary'])->dict:
...
class Extractor:
    ...
    def extract_values(self,result:DeepchecksParsedResult)->Result:
        ...
        for cr in json_result.get("conditions_results",[]):
            ...
            conditions_results.append(ConditionResult(status=status,
                                    condition=condition,
                                    more_info=more_info
                                )
            )
        ...
        return Result(check=check_name,params=params,
                summary=summary,
                value=value,
...
class DeepchecksPromptBuilder(BasePromptBuilder):
    ...
    def build_prompt(
        ...
        if artifact.config:
            ...
            prompt_parts.append(f"- Train-test validation: {artifact.config.train_test_validation}")
            prompt_parts.append(f"- Data integrity checks: {artifact.config.data_integrity}")
            prompt_parts.append(f"- Model evaluation: {artifact.config.model_evaluation}")
        ...
        for k,v in artifact.results.items():
            prompt_parts.append(f"\n{k}:")
            for result in v:
                extracted_result = extractor.extract_values(result).to_dict(exclude_fields_from_result)
                if len(extracted_result.get("conditions_results",[]))==0 and exclude_empty_conditions_results:
                    continue
                else:
                    prompt_parts.append(f"- {OmegaConf.to_yaml(extracted_result,resolve=True)}")
