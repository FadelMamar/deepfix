...
class IntelligenceClient:
    ...
    def _initialize_providers(self) -> None:
        ...
        # Coding Agent(s)
        agent_cfg = (self.config.get(ProviderType.CODING_AGENT.value) or {}).get(
            "cursor"
        ) or {}
    ...
    def execute_query(
        ...
        with Progress() as progress:
            response = provider.execute(prompt, context or {})

        return response

    def _select_provider(
        self, provider_type: ProviderType, provider_name: Optional[Providers] = None
    ) -> CursorAgentProvider | DspyLLMProvider:
        if provider_name:
        ...
        raise IntelligenceProviderError(
            f"No provider available for type: {provider_type}"
        )
