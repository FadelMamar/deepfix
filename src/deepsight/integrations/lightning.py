...
class DeepSightCallback(Callback):
    def __init__(self,config: dict,dataset_name:str,deepchecks_config: DeepchecksConfig,train_dataset:Dataset,val_dataset:Optional[Dataset] = None):
        super().__init__()
    ...
    @property
    def state(self):
        return dict(mlflow_run_id=self.mlflow_run_id, 
    ...
    def run_deepchecks(self,pl_module:L.LightningModule) -> None:
        ...
        self.train_dataset, self.val_dataset = self.load_vision_data(train_data=self.train_dataset,
                                                                val_data=self.val_dataset,
                                                                batch_size=self.deepchecks_config.batch_size,
                                                                model=pl_module.predict_step
                                                            )
        artifacts = deepchecks_runner.run_suites(train_data=self.train_dataset, 
    ...
    def load_vision_data(self,train_data:Dataset,batch_size:int,
                        ...
                        model:Optional[Callable[[torch.Tensor],torch.Tensor]] = None) -> Tuple[VisionData,VisionData]:
        train_data = ClassificationVisionDataLoader.load_from_dataset(train_data,batch_size=batch_size,model=model)
        if val_data is not None:
            val_data = ClassificationVisionDataLoader.load_from_dataset(val_data,batch_size=batch_size,model=model)
