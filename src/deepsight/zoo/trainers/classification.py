...
class ClassificationTrainerConfig(BaseModel):
    ...
    label_to_class_map: Dict[int, str] = Field(default={},description="Label to class map")
    ...
    # Mlflow
    experiment_name: str = Field(default="classification",description="Experiment name")
    run_name: str = Field(default="default",description="Run name")
    log_best_model: bool = Field(default=True,description="Log best model")
    tracking_uri: str = Field(default="http://localhost:5000",description="Tracking URI")
...
class ClassificationDataModule(L.LightningDataModule):
    def __init__(self, train_dataset: Dataset,val_dataset: Dataset,batch_size: int = 16,num_workers: int = 4,pin_memory: bool = False):
        super().__init__()
...
class ClassificationTrainer(object):
    ...
    def run(self, model:torch.nn.Module,
        ...
        # DataModule 
        datamodule = ClassificationDataModule(train_dataset=train_dataset,
