...
class FoodWastedatasetWithEmbeddings(FoodWasteDataset):
    def __init__(self,
        ...
        super().__init__(ing2label=ing2label,ing2name=ing2name,split=split,image_size=image_size)
    ...
    def compute_embeddings(self,num_workers:int=4,batch_size:int=8,pin_memory:bool=True,device:str="cpu"):
        self.embedding_model = self.embedding_model.to(device)

        loader = DataLoader(self,batch_size=batch_size,shuffle=False,num_workers=num_workers,pin_memory=pin_memory)
        for batch_image,batch_label in tqdm(loader,desc="Computing embeddings",total=int(len(self)/batch_size)):
        ...
        self.dataset_embeddings = torch.stack(self.dataset_embeddings,dim=0)
        self.dataset_labels = torch.stack(self.dataset_labels,dim=0)
        self._embeddings_computed = True
        return None
    
    def __getitem__(self, idx):
        if not self._embeddings_computed:
            # Return parent class behavior when embeddings not computed
            return super().__getitem__(idx)
        return self.dataset_embeddings[idx], self.dataset_labels[idx]
    
    def __len__(self):
        if not self._embeddings_computed:
            # Return parent class length when embeddings not computed
    ...
    def from_embeddings(cls,path:str):
        ...
        dataset = cls(embedding_model=None,ing2label=ing2label,ing2name=ing2name,split=split,image_size=image_size)
        dataset.dataset_embeddings = torch.load(embeddings_path,map_location="cpu",mmap=True)
...
def create_classification_dataset(ing2label:OrderedDict,
                                    ing2name:OrderedDict,
                                    split='train',
                                    image_size:int=1024,
                                    embedding_model:Optional[FeatureExtractor]=None,
    ...
    if embedding_model is not None:
        dataset = FoodWastedatasetWithEmbeddings(ing2label=ing2label,ing2name=ing2name,split=split,image_size=image_size,embedding_model=embedding_model)
        dataset.compute_embeddings(device=device,num_workers=num_workers,batch_size=batch_size,pin_memory=pin_memory)
        return dataset
    else:
        return FoodWasteDataset(ing2label=ing2label,ing2name=ing2name,split=split,image_size=image_size)

def load_train_and_val_datasets(image_size:int=1024,
                                embedding_model:Optional[FeatureExtractor]=None,
                                device:str="cpu",
                                num_workers:int=4,
                                batch_size:int=8,
    ...
    train_dataset = create_classification_dataset(ing2label=ing2label,ing2name=ing2name,split='train',**cfg)
    val_dataset = create_classification_dataset(ing2label=ing2label,ing2name=ing2name,split='test',**cfg)
